...
function outputToGitHub(settings) {
  ...
  return through(function (item) {
    ...
    if ((i++) % 200 === 0) console.log(item.header.date);
    var self = this;

    if (10 < ++inProgress) self.pause();
...
function current(date) {
  ...
  if (date.getFullYear() === (now.getFullYear() - 1) && date.getMonth() === 11 && now.getMonth() === 0) return true;
  return false;
}