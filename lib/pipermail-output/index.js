...
function outputToDir(dir, github) {
  ...
  return transform(function (item, finish) {
    ...
    function next(i) {
      if (i === folders.length) return finish();
      if (modified[folders[i]]) {
        console.log('add: ' + join(dir, folders[i]));
        spawn('git',['add', '-A'], {cwd: join(dir, folders[i]), stdio: 'inherit'})
          .on('exit', function () {
            console.log('commit: ' + join(dir, folders[i]));
            spawn('git',['commit', '-m', 'Add messages'], {cwd: join(dir, folders[i]), stdio: 'inherit'})
              .on('exit', function () {
                console.log('push: ' + join(dir, folders[i]));
                spawn('git push origin master', [], {cwd: join(dir, folders[i]), stdio: 'inherit'})
                  .on('exit', function () {
                    next(i + 1);
                  });
              });
