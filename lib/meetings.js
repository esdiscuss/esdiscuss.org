...
var githubUser = process.env.GITHUB_USER || 'JacksonScript';
var githubRepo = process.env.GITHUB_REPO || 'meetings';
var githubBranch = process.env.GITHUB_BRANCH || 'master';

var meetings = new Repository(githubUser, githubRepo, null, {
  updateFrequency: '10m',
  retryFrequency: '2m',
  branch: githubBranch
});
  ...
  return meetings.ready.done(function () {
    ...
    if (entry) {
      var event = new icalevent({
        offset:moment(entry.location.timeStart).zone(),
        start: moment.utc(entry.location.timeStart).format(),
        end: moment.utc(entry.location.timeEnd).format(),
...
var upcoming = function upcoming() {
  var futureMeetups = [];
  var sorted = meetups.sort(function(a,b){
    if (a.meetupDay < b.meetupDay) return -1;
    if (a.meetupDay > b.meetupDay) return 1;
    return 0;
  });
  sorted.forEach(function(meetup){
...
module.exports = { app: app, pages: pages, upcoming: upcoming };
