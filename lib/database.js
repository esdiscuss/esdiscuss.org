...
function connect() {
  return mongod(user + ':' + pass + '@ds049798-a0.mongolab.com:49798/esdiscuss',
...
exports.botRuns = function (date) {
  ...
  var end = new Date((+start) + ms('1day'));

  console.dir(start.toISOString());
  console.dir(end.toISOString());
  var filter = {
    type: 'bot-run',
    start: {
      '$gt': start,
  ...
  }).sort({start: -1}).then(function (res) {
    return res.map(function (run) {
      return {
        start: run.start,
