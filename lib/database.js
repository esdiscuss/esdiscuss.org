  ...
  return Q.promise(function (resolve, reject) {
    ...
    .then(function (res) {
      ...
      var headers = Q.promise(function (resolve, reject) {
        db.headers.find({subjectID: res.subjectID}).sort({date: 1}, function (err, res) {
          if (err) return reject(err);
          else return resolve(res)
        })
      })
      var contents = Q.promise(function (resolve, reject) {
        db.contents.find({subjectID: res.subjectID}, function (err, res) {
          if (err) return reject(err);
          else return resolve(res)
        })
      })
...
exports.getNewLocation = function (oldSubjectID) {
  return Q.promise(function (resolve, reject) {
      db.topics.findOne({subjectID: oldSubjectID}, function (err, res) {
        if (err) return reject(err)
        else return resolve(res && res._id)
      })
    })
};
